          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                {config.key !== "historique" && <TableHead className="p-1"></TableHead>}
              </TableRow>
              <TableRow>
                {config.fields.map(field => (
                  <TableHead key={field.name} className="text-right">{field.label}</TableHead>
                ))}
                {config.key !== "historique" && <TableHead className="text-right">Actions</TableHead>}
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredItems.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={config.fields.length + (config.key !== "historique" ? 1 : 0)} className="text-center py-8">
